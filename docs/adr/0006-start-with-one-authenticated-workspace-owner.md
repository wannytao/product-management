...
Password change, Workspace import or export, Operations Evidence export, and security or network configuration changes require password re-authentication when the session's latest authentication proof is older than 15 minutes. Successful step-up authentication refreshes authentication age and rotates the opaque session token; ordinary product-domain commands continue to rely on the active session and their dedicated review or concurrency controls.
